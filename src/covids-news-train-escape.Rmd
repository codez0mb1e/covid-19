---
title: "Escape from COVID's News Train"
...
ggplot(data %>% filter_countries, aes(x = date)) +
  geom_col(aes(y = confirmed_n), alpha = .75) +
...
```{r warning=FALSE}
ggplot(data  %>% filter_countries, aes(x = date)) +
  geom_col(aes(y = confirmed_n_per_1M), alpha = .75) +
  scale_x_date(date_labels = "%d %b", date_breaks = "7 days") +
...
ggplot(plot_data, aes(x = since_1_confirmed_per_1M_date_n_days)) +
  ...
  geom_col(aes(y = n_per_1M, fill = case_state), alpha = .75) +
...
ggplot(plot_data, aes(x = since_1_confirmed_per_1M_date_n_days)) +
  ...
  geom_line(aes(y = active_n_per_1M, color = country, linetype = data_type)) +
