...
covid_data %>% 
  select(area, country_region, province_state, observation_date, confirmed) %>% 
...
spread_df <- covid_data %>% 
  group_by(
    country_region, observation_date
  ) %>% 
  summarise(
...
spread_df %>% 
  ...
  select(
    observation_date, starts_with("active_"), starts_with("confirmed_"), starts_with("recovered_"), starts_with("deaths_")
  )
```
...
ggplot(
  ...
  geom_hline(aes(yintercept = c(3.5e5)), linetype = "dashed", color = "black", alpha = .33) +
  annotate(geom = "text", label = "0.5% of world population", size = 3, color = "#666666", 
           x = min(spread_df$observation_date), y = 3.7e5, 
           vjust = "bottom", hjust = "left") +
  
  geom_hline(aes(yintercept = c(7e5)), linetype = "dashed", color = "black", alpha = .33) +
  annotate(geom = "text", label = "1% of world population", size = 3, color = "#666666",
           x = min(spread_df$observation_date), y = 7.2e5, 
           vjust = "bottom", hjust = "left") +
...
spread_df %>% 
  ...
  select(
    country_region, observation_date, starts_with("active_"), starts_with("confirmed_"), starts_with("recovered_"), starts_with("deaths_")
  )
```

```{r}
country_spread_df <- spread_df %>% 
    group_by(country_region) %>% 
    summarise_at(vars(ends_with("_total")), sum) %>% 
    ungroup %>% 
    top_n(10, confirmed_total)

country_spread_df$country_region <- factor(country_spread_df$country_region, 
                                           levels = country_spread_df$country_region[order(country_spread_df$confirmed_total)])


ggplot(
   country_spread_df %>% 
    gather( key = "case_state", value = "cases_n", -c(country_region, confirmed_total)),
  
  aes(x = country_region)) +
  
  geom_col(aes(y = confirmed_total), fill = "#0080FF", alpha = .25) +
  geom_col(aes(y = cases_n, fill = country_region), show.legend = F) +
  ...
  labs(x = "Country", y = "Number of Cases", 
       title = "COVID-19 Countries Spread", 
       subtitle = "Blue bar - all infected cases", 
       caption = lab_caption) +
...
ggplot(
  ...
  labs(title = "COVID-19 World Daily Spread", 
       subtitle = "World daily dynamics \nBlue line - infected cases minus recovered and fatal. Negative values indicate recovered and fatal cases", 
...
```{r}
countries_spread_daily <- spread_df %>% 
  ## calc cases per day stats
  mutate_at(
    vars(ends_with("_total")),
    list("per_day" = ~ (. - lag(.)))
...
ggplot(
    countries_spread_daily %>% 
      filter(country_region %in% countries_spread_list) %>% 
      transmute(
        country_region,
        observation_date,
      ...
      gather(
        key = "case_state", value = "cases_n", -c(observation_date, active_total_per_day, country_region)
      ), 
    aes(x = observation_date)
  ) +
  ...
  facet_grid(country_region ~ ., scales = "free") +
