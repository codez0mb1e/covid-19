...
ggplot(
  countries_spread %>% 
    ...
    filter(
      observation_date > Sys.Date() - weeks(2)
    ),
  aes(x = observation_date)) +
...
countries_mortality <- countries_spread %>% 
  ...
  filter(n_days_since_1st_deaths >= 0)
...
ggplot(
  ...
  labs(x = "Number of days since 1st fatal case", y = "Mortality rate", 
...
population_spread %>% 
  ...
  select(country, n_days_since_1st_confirmed, population, starts_with("confirmed_"))
```
...
population_spread_last_day %>% 
  ...
  select(
    ...
    n_days_since_1st_confirmed, n_days_since_1st_deaths
  )
```
...
observed_countries <- c(observed_countries, "Russia", "India", "Mainland China", "South Korea") %>% unique
observed_countries 
```


```{r warning=FALSE}
active_cases_since_1st_confirmed <- population_spread %>% 
  filter(country %in% observed_countries) %>% 
  left_join(
    population_spread_last_day %>% transmute(country, active_total_per_1M_last = active_total_per_1M, n_days_since_1st_confirmed),
...
ggplot(active_cases_since_1st_confirmed, aes(x = n_days_since_1st_confirmed)) +
  ...
  ylim(c(0, max(active_cases_since_1st_confirmed$active_total_per_1M) + 100)) +
...
rm(active_cases_since_1st_confirmed)
```
...
active_cases_since_1st_death <- population_spread %>% 
  filter(country %in% observed_countries) %>% 
  left_join(
    population_spread_last_day %>% transmute(country, active_total_per_1M_last = active_total_per_1M, n_days_since_1st_deaths),
...
ggplot(
  ...
  xlim(c(0, 42)) +
  ylim(c(0, max(active_cases_since_1st_death$active_total_per_1M) + 100)) +
