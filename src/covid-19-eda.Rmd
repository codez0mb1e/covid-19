...
ggplot(
  ...
  geom_hline(aes(yintercept = total_population*2e-3), linetype = "dashed", color = "black", alpha = .33) +
  annotate(geom = "text", label = "0.2% of world population", size = 3, color = "#666666", 
           x = min(world_spread$observation_date), y = (1.05*total_population*2e-3), 
...
ggplot(
  ...
  geom_hline(aes(yintercept = total_population*2e-3), linetype = "dashed", color = "black", alpha = .33) +
  annotate(geom = "text", label = "0.2% of world population", size = 3, color = "#666666", 
           x = Sys.Date() - weeks(4), y = (1.05*total_population*2e-3), 
           vjust = "bottom", hjust = "left") +
...
ggplot(
  ...
  labs(x = "Number of days since 10th fatal case", y = "Mortality rate (attention: different scales)", 
...
ggplot(
  ...
  annotate(geom = "text", label = "Mean mortality rate \n(over time, all countries)", x = 8, y = 1.1*mean(countries_mortality$confirmed_deaths_rate), vjust = 0, size = 3) +
...
ggplot(
  ...
  annotate(geom = "text", label = "Mean mortality rate \n(over time, all countries)", x = 8, y = 1.1*mean(countries_mortality$recovered_deaths_rate, na.rm = T), size = 3) +
...
observed_countries <- c(observed_countries, "Russia", "US", "United Kingdom", "Mainland China") %>% unique
observed_countries 
```


```{r active-cases-dynamics, warning=FALSE}
...
ggplot(active_cases_since_100st_confirmed, aes(x = n_days_since_100_confirmed)) +
  ...
  geom_line(aes(y = double_every_3d), linetype = "dotted", color = "red", alpha = .85) + 
  ...
  annotate(geom = "text", label = "Cases double \nevery 3 days", x = 30, y = 2800, vjust = 0, size = 3) +
  annotate(geom = "text", label = "...every week", x = 60, y = 3100, vjust = 0, size = 3) +
